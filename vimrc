...
set ttimeoutlen=50          " Escape rápido en terminal
...
set wildignore+=*.o,*.obj,*.pyc,*.class,*.jar,*.swp,*.swo
set wildignore+=*.jpg,*.png,*.gif,*.ico,*.pdf,*.DS_Store
...
" clipboard=unnamed removed: y uses internal register only
" Use <Leader>y / <Leader>p for system clipboard (see mappings below)
...
" Custom filetype associations (only types Vim 9.1 doesn't detect natively)
...
function! s:GrepWithHighlight()
  let l:pattern = input("Grep: ")
  if empty(l:pattern)
    return
  endif
  execute "grep -R " . l:pattern . " ."
  copen
  let @/ = l:pattern
  set hlsearch
endfunction
nnoremap <Leader>F :call <SID>GrepWithHighlight()<CR>
...
" Comment toggle function (adds or removes comments)
function! s:GetCommentStr()
    let l:ft = &filetype
    if l:ft =~# '\v^(javascript|typescript|typescriptreact|javascriptreact|c|cpp|java|rust|go)'
        return '// '
    elseif l:ft =~# '\v^(python|ruby|sh|bash|zsh|yaml|terraform|conf)'
        return '# '
    elseif l:ft ==# 'vim'
        return '" '
    elseif l:ft ==# 'lua'
        return '-- '
    elseif l:ft =~# '\v^(css|scss)'
        return '/* '
    endif
    return '# '
endfunction
...
function! ToggleComment() range
    let l:cs = s:GetCommentStr()
    let l:ecs = escape(l:cs, '/".*[]~$^')
    let l:first_line = getline(a:firstline)
    if l:first_line =~# '^\s*' . l:ecs
        " Uncomment: remove first occurrence of comment string
        execute a:firstline . ',' . a:lastline . 's/^\(\s*\)' . l:ecs . '/\1/e'
    else
        " Comment: add comment string after leading whitespace
        execute a:firstline . ',' . a:lastline . 's/^\(\s*\)/\1' . l:ecs . '/e'
    endif
    nohlsearch
endfunction
...
" Highlight yanked text (Neovim only — lua call fails in Vim)
if has('nvim')
...
inoremap [ []<Left>
inoremap { {}<Left>
inoremap <expr> " <SID>SmartQuote('"')
inoremap <expr> ' <SID>SmartQuote("'")
inoremap <expr> ` <SID>SmartBacktick()

" Don't auto-pair quotes inside words (e.g. it's, don't)
function! s:SmartQuote(char)
    let l:col = col('.') - 1
    let l:prev = l:col > 0 ? getline('.')[l:col - 1] : ''
    if l:prev =~# '\w'
        return a:char
    endif
...
" Don't auto-pair backtick in markdown files
function! s:SmartBacktick()
...
" Jump out of brackets with Ctrl+L (Tab kept free for completion)
inoremap <expr> <C-l> search('\%#[]>)}''"`]', 'n') ? '<Right>' : '<C-l>'
...
"   Ctrl+L (insert mode)           - Jump out of bracket/quote
"
" ============================================================================
